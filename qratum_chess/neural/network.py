...
class PolicyValueNetwork:
    ...
    def _initialize_weights(self) -> None:
        ...
        # Initial convolution
        self.input_conv = np.random.randn(
            cfg.num_filters, cfg.num_input_channels, 3, 3
        ) * np.sqrt(2.0 / (cfg.num_input_channels * 9))
        self.input_bn_gamma = np.ones(cfg.num_filters)
        self.input_bn_beta = np.zeros(cfg.num_filters)
        
        # Residual blocks
        self.residual_blocks = [
            ResidualBlock(cfg.num_filters)
        ...
        self.policy_fc2 = np.random.randn(
            cfg.policy_fc_size, cfg.num_moves
        ) * np.sqrt(2.0 / cfg.policy_fc_size)
        
        # Value head
        self.value_conv = np.random.randn(
            cfg.value_head_filters, cfg.num_filters, 1, 1
        ) * np.sqrt(2.0 / cfg.num_filters)
        self.value_bn_gamma = np.ones(cfg.value_head_filters)
        self.value_bn_beta = np.zeros(cfg.value_head_filters)
        self.value_fc1 = np.random.randn(
            cfg.value_head_filters * 64, cfg.value_fc_size
        ) * np.sqrt(2.0 / (cfg.value_head_filters * 64))
