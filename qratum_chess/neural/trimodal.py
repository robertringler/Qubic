...
class TacticalCortex:
    ...
    def _score_move_tactically(self, position: Position, move: Move, base_eval: float) -> float:
    ...
    def _calculate_confidence(self, position: Position, move_values: dict) -> float:
        ...
        return CortexOutput(
            ...
            features={
                ...
                "novelty_score": np.mean(
                    [self._calculate_novelty(position, m) for m in legal_moves]
                ),
            },
        )
            ...
            for pt in PieceType:
                ...
                for sq in iter_bits(pieces):
                    ...
                    for i in range(8, 24):
                        ...
                        embedding[i + 16] = (
                            np.sin(file * freq) if i % 2 == 0 else np.cos(file * freq)
                        )
...
class TriModalCore:
    ...
    def evaluate(
        self, position: Position, legal_moves: list
    ...
    def _compute_adaptive_weights(
        ...
        if total_conf > 0:
            ...
            alpha = 0.5
            weights = {
                k: alpha * base_weights[k] + (1 - alpha) * conf_weights[k] for k in base_weights
            }
        else:
    ...
    def _compute_novelty_pressure(self, position: Position, move: Move) -> float:
        ...
        recent_count = sum(1 for pos_hash, m in self.move_history[-100:] if m == move_key)
