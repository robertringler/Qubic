...
class NetworkTrainer:
    ...
    def __init__(self, network, config: TrainingConfig | None = None):
    ...
    def train_batch(self, samples: list[GameSample], encoder: PositionEncoder) -> TrainingMetrics:
    ...
    def _update_weights(
        self, samples: list, positions: list, encoder, policy_out: np.ndarray, value_out: np.ndarray
    ) -> None:
        ...
        if error_magnitude > 0:
            ...
            self.network.value_fc2 += (
                lr * 0.001 * error_magnitude * np.random.randn(*self.network.value_fc2.shape)
            )
    ...
    def train_epoch(self, samples: list[GameSample], encoder: PositionEncoder) -> TrainingMetrics:
    ...
    def train(
        self, samples: list[GameSample], encoder: PositionEncoder, epochs: int | None = None
