...
class KnowledgeDistillation:
    ...
    def detect_innovation(
        self, student_network, test_positions: list, encoder
    ) -> tuple[bool, list[str]]:
    ...
    def get_teacher_ensemble(self) -> list[GenerationRecord]:
        ...
        sorted_gens = sorted(self.generation_history, key=lambda g: g.elo_rating, reverse=True)

        return sorted_gens[:3]

    def validate_promotion(
        self, student_elo: float, test_positions: list, encoder, student_network
    ) -> tuple[bool, str]:
        ...
        if self.best_generation:
            if student_elo < self.best_generation.elo_rating - 10:
                return (
                    False,
                    f"Elo regression: {student_elo} < {self.best_generation.elo_rating - 10}",
                )

        # Check innovation
        is_innovative, new_motifs = self.detect_innovation(student_network, test_positions, encoder)
