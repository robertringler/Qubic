...
class BenchmarkRunner:
    ...
    def run(self, engine, evaluator=None) -> BenchmarkSummary:
        ...
        if self.config.run_resilience:
            ...
            summary.resilience = self.resilience.run_full_suite(
                engine, evaluator, self.test_positions, iterations=self.config.resilience_iterations
            )
    ...
    def _simulate_game_results(self, torture_report: TortureSuiteReport) -> list[tuple[str, float]]:
        """Simulate game results from torture suite performance.
    ...
    def print_summary(self, summary: BenchmarkSummary) -> None:
        ...
        if summary.elo:
            ...
            print(
                f"  Confidence: [{summary.elo.elo_rating.confidence_low:.0f}, {summary.elo.elo_rating.confidence_high:.0f}]"
            )
        ...
        if summary.certification:
            ...
            print(
                f"  Overall Certified: {'✓ PASS' if summary.certification.overall_certified else '✗ FAIL'}"
            )
            print(
                f"  Stockfish-NNUE Winrate: {summary.certification.stockfish_winrate*100:.1f}% "
                f"(target: ≥75%) {'✓' if summary.certification.stockfish_pass else '✗'}"
            )
            print(
                f"  Lc0-class Winrate: {summary.certification.lc0_winrate*100:.1f}% "
                f"(target: ≥70%) {'✓' if summary.certification.lc0_pass else '✗'}"
            )
            print(
                f"  Novel Motif Emergence: {'Confirmed ✓' if summary.certification.motif_emergence else 'Not Detected'}"
            )
    ...
    def _export_metrics_csv(self, filepath: Path, summary: BenchmarkSummary) -> None:
        ...
        if summary.torture:
            rows.append(
                [
                    "torture",
                    "eval_volatility",
                    f"{summary.torture.eval_volatility:.6f}",
                    "ratio",
                    "≤0.05",
                    "PASS" if summary.torture.eval_volatility <= 0.05 else "FAIL",
                ]
            )
            rows.append(
                [
                    "torture",
                    "blunder_rate",
                    f"{summary.torture.blunder_rate:.6f}",
                    "ratio",
                    "≤0.0001",
                    "PASS" if summary.torture.blunder_rate <= 0.0001 else "FAIL",
                ]
            )
            rows.append(
                [
                    "torture",
                    "positions_tested",
                    str(summary.torture.positions_tested),
                    "count",
                    "N/A",
                    "N/A",
                ]
            ...
            rows.append(
                [
                    "torture",
                    "correct_moves",
                    str(summary.torture.correct_moves),
                    "count",
                    "N/A",
                    "N/A",
                ]
        ...
        if summary.elo:
            rows.append(
                ["elo", "rating", f"{summary.elo.elo_rating.rating:.0f}", "elo", "N/A", "N/A"]
            )
            rows.append(
                [
                    "elo",
                    "confidence_low",
                    f"{summary.elo.elo_rating.confidence_low:.0f}",
                    "elo",
                    "N/A",
                    "N/A",
                ]
            ...
            rows.append(
                [
                    "elo",
                    "confidence_high",
                    f"{summary.elo.elo_rating.confidence_high:.0f}",
                    "elo",
                    "N/A",
                    "N/A",
                ]
            ...
            rows.append(
                [
                    "elo",
                    "is_promoted",
                    str(summary.elo.is_promoted),
                    "bool",
                    "True",
                    "PASS" if summary.elo.is_promoted else "FAIL",
                ]
            )
        ...
        if summary.resilience:
            rows.append(
                [
                    "resilience",
                    "passed_tests",
                    str(summary.resilience.passed_tests),
                    "count",
                    "N/A",
                    "N/A",
                ]
            ...
            rows.append(
                [
                    "resilience",
                    "failed_tests",
                    str(summary.resilience.failed_tests),
                    "count",
                    "0",
                    "PASS" if summary.resilience.failed_tests == 0 else "FAIL",
                ]
            )
            rows.append(
                [
                    "resilience",
                    "avg_recovery_ms",
                    f"{summary.resilience.average_recovery_ms:.2f}",
                    "ms",
                    "≤500",
                    "PASS" if summary.resilience.average_recovery_ms <= 500 else "FAIL",
                ]
            )
        ...
        if summary.certification:
            rows.append(
                [
                    "certification",
                    "stockfish_winrate",
                    f"{summary.certification.stockfish_winrate:.4f}",
                    "ratio",
                    "≥0.75",
                    "PASS" if summary.certification.stockfish_pass else "FAIL",
                ]
            )
            rows.append(
                [
                    "certification",
                    "lc0_winrate",
                    f"{summary.certification.lc0_winrate:.4f}",
                    "ratio",
                    "≥0.70",
                    "PASS" if summary.certification.lc0_pass else "FAIL",
                ]
            )
            rows.append(
                [
                    "certification",
                    "motif_emergence",
                    str(summary.certification.motif_emergence),
                    "bool",
                    "True",
                    "PASS" if summary.certification.motif_emergence else "FAIL",
                ]
            )
    ...
    def _generate_html_report(
        self, filepath: Path, summary: BenchmarkSummary, timestamp: str
    ) -> None:
...
"""
    ...
    def certify_against_stage_iii(
        ...
        if motif_detected is not None:
            ...
            summary.torture
            and summary.torture.correct_moves >= summary.torture.positions_tested * 0.9
        ):
    ...
    def run_single_game(self, engine) -> dict[str, Any]:
        ...
        while move_count < max_moves:
            ...
            if self.config.run_telemetry:
                self.telemetry.record_time_per_move(stats.time_ms if stats else 0.0)
                self.telemetry.record_novelty_pressure(telemetry.get("novelty_pressure", 0.0))
    ...
    def compile_full_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "elo": {
                ...
                "delta": (
                    (elo_progression[-1] - elo_progression[0]) if len(elo_progression) > 1 else 0.0
                ),
