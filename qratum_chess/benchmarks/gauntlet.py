...
class GauntletReport:
    ...
    def finalize(self) -> None:
        ...
        self.losses = sum(1 for g in self.games if g.winner and g.winner != self.engine_name)
    ...
    def passed(self) -> bool:
        ...
        return (
            self.vs_alphabeta_winrate >= 0.75
            and self.vs_neural_winrate >= 0.70
            and self.vs_human_alignment >= 0.85
        )
...
class AdversarialGauntlet:
    ...
    def register_adversary(self, adversary_type: AdversaryType, adversary: Any) -> None:
            ...
            for ply in range(max_ply):
                ...
                try:
                    best_move, _, _ = current_engine.search(
                        position, time_limit_ms=min(time_control_ms / 40, 1000)
                    )
                except Exception:
                    best_move = None
    ...
    def run_full_gauntlet(self, engine, games_per_adversary: int = 100) -> GauntletReport:
        ...
        for adv_type, adversary in self.adversaries.items():
            results = self.run_match(
                engine, adversary, f"{adv_type.value}_adversary", num_games=games_per_adversary
            )
        ...
        for pos_data in kaggle_positions:
            ...
            try:
                ...
                engine_move, eval_score, stats = engine.search(
                    position, depth=depth, time_limit_ms=time_limit_ms
                )
                elapsed = (time.perf_counter() - start) * 1000
    ...
    def run_kaggle_benchmark_adversary(
        self, engine, leaderboard_file: str, depth: int = 15, max_positions: int | None = None
    ) -> float:
        ...
        try:
            ...
            results = runner.run_benchmark(
                engine, leaderboard, depth=depth, max_positions=max_positions
            )
