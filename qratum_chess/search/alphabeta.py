...
class AlphaBetaSearch:
    ...
    def __init__(
        ...
        self.killer_moves: list[list[Move | None]] = [[None, None] for _ in range(100)]
    ...
    def search(
        self, position: Position, depth: int | None = None, time_limit_ms: float | None = None
    ...
    def _alphabeta_root(
        self, position: Position, depth: int, alpha: float, beta: float, legal_moves: list[Move]
    ) -> float:
    ...
    def _quiescence(self, position: Position, alpha: float, beta: float, q_depth: int) -> float:
        """Quiescence search for tactical stability."""
        ...
        legal_moves = position.generate_legal_moves()
        captures = [m for m in legal_moves if position.board.piece_at(m.to_sq) or m.is_en_passant]

        # Order captures by MVV/LVA
        captures.sort(key=lambda m: self._mvv_lva_score(position, m), reverse=True)
    ...
    def _store_tt(
        self, position: Position, depth: int, value: float, flag: str, best_move: Move | None
    ) -> None:
