...
from qratum_chess.search.mcts import MCTSSearch, MCTSConfig
...
class AsymmetricAdaptiveSearch:
    ...
    def _middlegame_search(
        ...
        best_move, value, ab_stats = alphabeta.search(
            position,
            time_limit_ms=time_limit_ms
        )
        ...
        if entropy > 2.0 and time_limit_ms and time_limit_ms > 100:
            ...
            mcts_move, _, mcts_value = self.mcts.search(
                position,
                time_limit_ms=verification_time
            )
    ...
    def _endgame_search(
        ...
        alphabeta = AlphaBetaSearch(
            evaluator=self._endgame_evaluator,
            config=ab_config
        )
    ...
    def _probe_tablebase(
        self,
        position: Position
    ) -> tuple[Move | None, float, AASStats] | None:
