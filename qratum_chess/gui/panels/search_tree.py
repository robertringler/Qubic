...
class SearchTreePanel:
    ...
    def update_node(self, node_id: int, visits: int, value: float, policy_prob: float = 0.0) -> None:
    ...
    def _compute_radial_layout(self) -> None:
        ...
        def assign_radial(node_id: int, angle_start: float, angle_end: float, radius: float) -> None:
            node = self.state.nodes[node_id]
    ...
    def _compute_force_directed_layout(self) -> None:
        ...
        for _ in range(50):
            forces: dict[int, tuple[float, float]] = {nid: (0.0, 0.0) for nid in self.state.nodes}
    ...
    def get_render_data(self) -> dict[str, Any]:
        ...
        for node_id, node in self.state.nodes.items():
            ...
            size = 10 + (node.visits / max(1, max(n.visits for n in self.state.nodes.values()))) * 20 if self.state.node_size_by_visits else 15
        ...
        for node_id, node in self.state.nodes.items():
            if node.parent_id is not None:
                ...
                if parent and parent.is_expanded:
                    is_pv = node_id in self.state.highlighted_path and node.parent_id in self.state.highlighted_path
                    edges_data.append({
            ...
            if node:
                selected_details = {
                    ...
                    'children': [
                        ...
                        for c in node.children if c in self.state.nodes
                    ],
                }
