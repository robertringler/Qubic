...
class TelemetryPanel:
    ...
    def record_recovery(self, recovery_time_ms: float) -> None:
        ...
        self.state.recovery.max_recovery_time_ms = max(
            self.state.recovery.max_recovery_time_ms, recovery_time_ms
        )
    ...
    def update_compliance(
        ...
        """
        self.state.compliance.iso_status = (
            ComplianceStatus.COMPLIANT if iso_compliant else ComplianceStatus.NON_COMPLIANT
        )
        self.state.compliance.nist_status = (
            ComplianceStatus.COMPLIANT if nist_compliant else ComplianceStatus.NON_COMPLIANT
        )
        self.state.compliance.eu_ai_status = (
            ComplianceStatus.COMPLIANT if eu_ai_compliant else ComplianceStatus.NON_COMPLIANT
        )
    ...
    def get_render_data(self) -> dict[str, Any]:
        ...
        perf_data = {
            ...
            "nps_percent_single": (
                min(100, perf.nodes_per_second / perf.target_nps_single * 100)
                if perf.target_nps_single
                else 0
            ),
        ...
        memory_data = {
            ...
            "heap_percent": (
                (mem.heap_used_mb / mem.heap_total_mb * 100) if mem.heap_total_mb else 0
            ),
