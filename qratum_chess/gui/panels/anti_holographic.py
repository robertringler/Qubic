...
class AntiHolographicPanel:
    ...
    def update_stochasticity(
        ...
        current_stoch = 0.4 * normalized_entropy + 0.4 * rank_stochasticity + 0.2 * (1 - min(1, variance * 10))
        ...
        if len(self.state.stochasticity.stochasticity_history) > 100:
            self.state.stochasticity.stochasticity_history = self.state.stochasticity.stochasticity_history[-100:]
    ...
    def update_destabilization(
        ...
        self.state.destabilization.time_pressure_factor = max(0, min(1, opponent_time_used_ratio - 1))
        ...
        # Determine level
        for level, threshold in sorted(self.DESTABILIZATION_THRESHOLDS.items(), key=lambda x: x[1], reverse=True):
        ...
        if len(self.state.destabilization.destabilization_history) > 100:
            self.state.destabilization.destabilization_history = self.state.destabilization.destabilization_history[-100:]
    ...
    def update_anti_reconstructibility(
        ...
        if len(self.state.anti_reconstructibility.history) > 100:
            self.state.anti_reconstructibility.history = self.state.anti_reconstructibility.history[-100:]
    ...
    def get_render_data(self) -> dict[str, Any]:
        ...
        drift_data = {
            ...
            'direction_color': self.COLORS['positive'] if self.state.drift.drift_direction > 0 else self.COLORS['negative'],
        }
