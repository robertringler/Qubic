...
class SubmissionFormatter:
    ...
    def format_submission(
        ...
        with open(output_path, 'w', newline='') as f:
            ...
            if metadata:
                ...
                f.write(f"# CORTEX_WEIGHTS={','.join(f'{k}={v}' for k, v in metadata.cortex_weights.items())}\n")
    ...
    def convert_engine_output(
        ...
        for (pos_id, position), move, eval_score in zip(positions, moves, evaluations):
            predictions.append({
                "id": pos_id,
                "move": move.to_uci() if move else "0000",
...
def create_submission_metadata(
    ...
    cortex_weights = config.get("cortex_weights", {
        "tactical": 0.33,
        "strategic": 0.33,
        "conceptual": 0.34
    })
