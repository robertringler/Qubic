...
class EquivalenceValidator:
    ...
    def validate_array_equivalence(
        ...
        return {
            ...
            "reason": None if equivalent else f"Max difference {max_diff:.2e} exceeds tolerance {self.tolerance:.2e}"
        }
    ...
    def validate_scalar_equivalence(
        ...
        return {
            ...
            "diff": float(diff),
            "tolerance": self.tolerance,
            "reason": None if equivalent else f"Difference {diff:.2e} exceeds tolerance {self.tolerance:.2e}"
        }
    ...
    def validate_dict_equivalence(
        ...
        for key in keys_to_check:
            ...
            if isinstance(val1, np.ndarray) and isinstance(val2, np.ndarray):
                result = self.validate_array_equivalence(val1, val2, f"{key}[dict1]", f"{key}[dict2]")
            elif isinstance(val1, (int, float)) and isinstance(val2, (int, float)):
                result = self.validate_scalar_equivalence(val1, val2, f"{key}[dict1]", f"{key}[dict2]")
            else:
