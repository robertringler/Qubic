...
def run_benchmark(config: BenchmarkConfig) -> BenchmarkResult:
    ...
    memory_ratio = (
        ...
        if dijkstra_metrics["memory_mb"] > 0
        else 1.0
    )
...
def run_benchmark_suite(
    configs: list[BenchmarkConfig], output_file: str | None = None
...
def main():
    ...
    parser = argparse.ArgumentParser(description="PostDijkstra SSSP Benchmark Suite")
    ...
    parser.add_argument(
        "--custom", action="store_true", help="Run custom benchmark instead of full suite"
    )

    parser.add_argument(
        "--nodes", type=int, default=50000, help="Number of nodes for custom benchmark"
    )

    parser.add_argument(
        "--edge-prob", type=float, default=0.0002, help="Edge probability for custom benchmark"
    )
