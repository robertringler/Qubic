...
class BucketedFrontier:
    ...
    def is_empty(self) -> bool:
        ...
        return self.min_bucket_id is None or all(not b.nodes for b in self.buckets.values())
...
class PostDijkstraSSSP:
    ...
    def _delta_stepping(
        self, source: int, coarse_bounds: list[float] | None, metrics: PostDijkstraMetrics
    ) -> list[float]:
            ...
            for neighbor, weight in self.graph.neighbors(node):
                ...
                if actual > expected + 1e-6:
                    ...
                    if violations <= 3:  # Only print first few
                        print(
                            f"Warning: Potential suboptimal path {node}->{neighbor}: "
