...
class SimulationTrace:
    ...
    def from_dict(cls, data: dict[str, Any]) -> SimulationTrace:
        ...
        trace.state_snapshots = [
            QuantumStateFingerprint.from_dict(s) for s in data.get("state_snapshots", [])
        ]
        return trace
...
class DeterministicSimulator:
    ...
    def __init__(self, seed: int, num_qubits: int = 4, precision: str = "fp64"):
    ...
    def _get_gate_matrix(self, gate: str, params: dict[str, Any]) -> np.ndarray:
        ...
        if gate == "H":
            ...
            theta = params.get("theta", 0)
            return np.array(
                [[np.exp(-1j * theta / 2), 0], [0, np.exp(1j * theta / 2)]], dtype=self._state.dtype
            )
        else:
