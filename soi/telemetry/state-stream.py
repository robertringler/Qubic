...
class SOITelemetryServer:
    ...
    def __init__(self):
        ...
        self.state = {
            "validators": {"total": 256, "active": 243, "pending": 8, "jailed": 3, "slashed": 2},
            "consensus": {
                "height": 1847293,
                "round": 0,
                "phase": "FINALIZED",
            ...
            "zones": {"Z0": 847, "Z1": 432, "Z2": 156, "Z3": 23},
            "trajectory": {
                "health_score": 0.98,
            ...
            "merkle": {"root": self._generate_hash(), "chain_length": 1847293},
        ...
        while self.is_running:
            try:
                ...
                event = self.create_event(
                    "qradle:state",
                    {"merkle": self.state["merkle"].copy(), "epoch": self.current_epoch},
                )
                ...
                event = self.create_event("aethernet:consensus", self.state["consensus"].copy())
                await self.broadcast(event)
                ...
                event = self.create_event(
                    "aethernet:validator", {"stats": self.state["validators"].copy()}
                )
                await self.broadcast(event)
                ...
                percentage = (
                    self.state["consensus"]["quorum_power"]
                    / self.state["consensus"]["total_power"]
                    * 100
                )
                ...
                self.state["trajectory"]["collapse_probability"] = max(
                    0,
                    min(
                        0.01,
                        self.state["trajectory"]["collapse_probability"]
                        + (random.random() - 0.5) * 0.001,
                    ),
                )
                self.state["trajectory"]["health_score"] = max(
                    0.9, min(1.0, 1.0 - self.state["trajectory"]["collapse_probability"] * 5)
                )

                event = self.create_event("trajectory:health", self.state["trajectory"].copy())
                await self.broadcast(event)
                ...
                event = self.create_event("telemetry:zone", {"zones": self.state["zones"].copy()})
                await self.broadcast(event)
    ...
    def get_state(self) -> dict:
        ...
        return {"epoch": self.current_epoch, "timestamp": self._get_timestamp(), **self.state}
            ...
            while True:
                ...
                if message.get("command") == "ping":
                    ...
                    await websocket.send_json(
                        {"type": "state", "payload": telemetry_server.get_state()}
                    )

        except WebSocketDisconnect:
            telemetry_server.clients.remove(websocket)
            print(
                f"[SOI Telemetry] Client disconnected ({len(telemetry_server.clients)} remaining)"
            )
