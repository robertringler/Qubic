...
def benchmark(cfg: BenchmarkConfig) -> Dict[str, Any]:
    ...
    for sigma in cfg.disorder_list:
        evolver = build_channel_evolver(cfg.Lx, cfg.Ly, A, B, gamma_center, cfg.T, cfg.dt, sigma, events)
    ...
    for rel in cfg.rel_variations:
        ...
        for sigma in cfg.disorder_list:
            evolver = build_channel_evolver(cfg.Lx, cfg.Ly, A, B, gamma, cfg.T, cfg.dt, sigma, events)
            outputs = logical_channel_pauli_action(evolver)
            chi = reconstruct_process_matrix(outputs)
    ...
    contrasts, visibility = run_ramsey(
        cfg.Lx, cfg.Ly, A, B, gamma_center, cfg.T, cfg.dt, cfg.disorder_list[0],
        phase_values=phase_values, t_phase=cfg.T / 2, phase_duration=cfg.phase_duration, delta_energy=delta,
    )
    ...
    N = cfg.Lx * cfg.Ly
    do_d2_otoc = bool(cfg.compute_d2_otoc and (cfg.force_d2_otoc or N <= 16) and (run_d2_otoc is not None))
    if do_d2_otoc:
        ...
        res = run_d2_otoc(Lx=cfg.Lx, Ly=cfg.Ly, disorder=cfg.disorder_list[0], T=cfg.T, gamma_list=gamma_list, compute_otoc_flag=True)
    ...
    if cfg.phys_freq_hz is not None and cfg.phys_freq_hz > 0:
        time_unit_sec = 1.0 / (2.0 * math.pi * float(cfg.phys_freq_hz))  # 1 model time unit in seconds
        ...
        dephase_limited_hz = (gamma_L_phys_hz / max(1e-12, cfg.clock_safety_factor)) if gamma_L > 0 else None
        ...
        physical = {
            ...
            'max_safe_clock_mhz': (max_safe_clock_hz/1e6 if max_safe_clock_hz is not None else None),
...
def write_artifacts(results: Dict[str, Any], out_prefix: str) -> None:
    ...
    lines.append(f"- Lattice: {results['config']['Lx']}x{results['config']['Ly']}  A={len(results['A'])} sites, B={len(results['B'])} sites")
    lines.append(f"- gamma_center: {results['gamma_center']:.5g}")
    lines.append(f"- F_proc (mean over sigma): {results['fproc_mean']:.4f}  | best: {results['fproc_best']:.4f}")
    lines.append(f"- Ramsey visibility (T={results['config']['T']}): {results['ramsey']['visibility']:.4f}  -> gamma_L proxy: {results['ramsey']['gamma_L_proxy']:.4g}")
    if results.get('physical', {}).get('enabled'):
        ...
        coh_us = (phys.get('coherence_time_seconds') * 1e6) if phys.get('coherence_time_seconds') else None
        lines.append(f"- Physical mapping: f={phys['freq_hz']:.4g} Hz; gate_time={gt_ns:.3g} ns; gamma_L={phys['gamma_L_phys_hz']:.3g} Hz; coherence≈{(coh_us if coh_us is not None else 'n/a')} μs; max_safe_clock≈{(max_clk_mhz if max_clk_mhz is not None else 'n/a'):.3g} MHz (κ={phys['clock_safety_factor']:.3g}).")
    if results['d2_otoc']['enabled']:
        ...
        lines.append(f"- D2(gamma_center): {d2 if d2 is not None else 'n/a'}  | OTOC(gamma_center): {oc if oc is not None else 'n/a'}  | gamma_opt_est: {go if go is not None else 'n/a'}")
    ...
    tex_lines = [
        ...
        (f"  \\item $D_2(\\gamma_c)={results['d2_otoc']['D2_at_gamma_center']:.4f}$, OTOC$ (\\gamma_c)={results['d2_otoc']['OTOC_at_gamma_center']:.4f}$, $\\gamma_\\mathrm{{opt}}\\approx{results['d2_otoc']['gamma_opt_est']:.4g}$."
         if results['d2_otoc']['enabled'] and results['d2_otoc']['D2_at_gamma_center'] is not None else None),
        (lambda: (f"  \\item Physical mapping $f={results['physical']['freq_hz']:.4g}$ Hz: gate time $\u200b{results['physical']['gate_time_ns']:.3g}$ ns; $\\Gamma_L\\approx{results['physical']['gamma_L_phys_hz']:.3g}$ Hz; max safe clock $\\approx{results['physical']['max_safe_clock_mhz']:.3g}$ MHz ($\\kappa={results['physical']['clock_safety_factor']:.3g}$).") if results.get('physical', {}).get('enabled') else None)(),
        "\\end{itemize}",
    ]
...
def print_tags(results: Dict[str, Any]) -> None:
    ...
    if results['d2_otoc']['enabled']:
        ...
        oc = results['d2_otoc']['OTOC_at_GAMMA_CENTER'] if 'OTOC_at_GAMMA_CENTER' in results['d2_otoc'] else results['d2_otoc'].get('OTOC_at_gamma_center')
        if oc is not None:
...
def parse_args() -> BenchmarkConfig:
    ...
    p.add_argument("--freq-hz", type=float, default=None, help="Characteristic frequency f (Hz); 1 model time unit = 1/(2π f) seconds")
    p.add_argument("--clock-safety", type=float, default=5.0, help="Safety factor κ for dephasing-limited clock")
    a = p.parse_args()
    cfg = BenchmarkConfig(
        Lx=a.Lx, Ly=a.Ly, block_size=a.block_size,
        disorder_list=a.disorder, gamma_center=a.gamma_center,
        gamma_C=a.gamma_C, gamma_alpha=a.gamma_alpha,
        rel_variations=a.rel, T=a.T, dt=a.dt,
        phase_duration=a.phase_duration, desired_phi=a.phi,
        ramsey_points=a.ramsey_points,
        compute_d2_otoc=(not a.no_d2_otoc), d2_otoc_gamma_points=a.d2_otoc_gamma_points,
