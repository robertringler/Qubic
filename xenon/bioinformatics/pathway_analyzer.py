...
class PathwayAnalyzer:
    ...
    def compute_pathway_enrichment(
        ...
        if background_genes is None:
            ...
            background_genes = list(set(
                gene for pathway in self._pathways.values()
                for reaction in pathway.reactions
        ...
        for pathway_id, pathway in self._pathways.items():
            ...
            pathway_genes = set(
                gene for reaction in pathway.reactions
                for gene in reaction.enzymes
            )
    ...
    def compute_metabolite_importance(
        ...
        connections: Dict[str, int] = {met: 0 for met in pathway.metabolites}
        ...
        importance = {
            met: count / max_connections
            for met, count in connections.items()
        }
