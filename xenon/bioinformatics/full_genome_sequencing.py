...
class GenomeSequencingConfig:
    ...
    fusion_constraints: ConservationConstraints = field(
        default_factory=ConservationConstraints
    )
...
class FullGenomeSequencingPipeline:
    ...
    def execute_transfer_entropy(
        self, timeseries_data: Dict[str, np.ndarray]
    ) -> Dict[str, Any]:
    ...
    def execute_inference(
        self, variant_graph: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def run(
        ...
        # Save all results
        self.save_results(
            alignment_results, fusion_results, te_results, inference_results
        )
