...
class TestFullGenomeSequencingPipeline:
    ...
    def test_deterministic_execution(self, pipeline_config, temp_output_dir):
        ...
        assert len(alignment_results1["alignments"]) == len(
            alignment_results2["alignments"]
        )
    ...
    def test_thread_safety(self, pipeline_config):
        ...
        result = pipeline.thread_safe_alignment.execute(
            "align", seq1[:50], seq2[:50]
        )
    ...
    def test_output_structure(self, pipeline_config):
        ...
        alignment_path = os.path.join(
            pipeline_config.output_dir, "alignment_result.json"
        )
        with open(alignment_path, "r") as f:
        ...
        inference_path = os.path.join(
            pipeline_config.output_dir, "functional_predictions.json"
        )
