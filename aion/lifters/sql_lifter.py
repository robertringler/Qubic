...
class SQLLifter:
    ...
    def _parse_select(self, tokens: list[str]) -> SQLASTNode:
        ...
        if i < len(tokens) and tokens[i] == "FROM":
            ...
            while i < len(tokens) and tokens[i] not in ("WHERE", "GROUP", "ORDER", "LIMIT", "JOIN", "LEFT", "RIGHT", "INNER", "OUTER"):
                if tokens[i] not in (",", "(", ")"):
        ...
        while i < len(tokens) and tokens[i] in ("JOIN", "LEFT", "RIGHT", "INNER", "OUTER"):
            ...
            if i < len(tokens):
                ...
                if i < len(tokens) and tokens[i] == "ON":
                    i += 1
                    condition = []
                    while i < len(tokens) and tokens[i] not in ("WHERE", "GROUP", "ORDER", "LIMIT", "JOIN", "LEFT", "RIGHT"):
    ...
    def _lift_from(self, node: SQLASTNode, provenance: Provenance, parent: Vertex | None) -> Vertex | None:
        """Lift FROM clause."""
    ...
    def _lift_where(self, node: SQLASTNode, provenance: Provenance, parent: Vertex | None) -> Vertex:
    ...
    def _lift_group_by(self, node: SQLASTNode, provenance: Provenance, parent: Vertex | None) -> Vertex:
    ...
    def _lift_order_by(self, node: SQLASTNode, provenance: Provenance, parent: Vertex | None) -> Vertex:
    ...
    def _lift_limit(self, node: SQLASTNode, provenance: Provenance, parent: Vertex | None) -> Vertex:
