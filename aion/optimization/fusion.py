...
from ..sir.vertices import Vertex, VertexType, HardwareAffinity, EffectKind
from ..sir.edges import HyperEdge, EdgeType, EdgeGroup, ParallelismKind
from ..sir.hypergraph import HyperGraph
from ..proof.verifier import ProofTerm, ProofKind
...
class KernelFusion:
        ...
        for pattern in self.patterns:
            ...
            if fusion_rule:
                ...
                if result.success:
                    ...
                    if proofs:
                        new_proofs = synthesize_proofs_for_rewrite(
                            graph, optimized, all_proofs
                        )
    ...
    def _fuse_python_rust(self, graph: HyperGraph, pattern: FusionPattern) -> FusionResult:
        ...
        python_calls = [v for v in pattern.vertices 
                        if v.metadata.provenance and v.metadata.provenance.source_language == "Python"]
        rust_calls = [v for v in pattern.vertices 
    ...
    def _fuse_cpu_gpu_transfer(self, graph: HyperGraph, pattern: FusionPattern) -> FusionResult:
        ...
        transfers = [v for v in pattern.vertices 
    ...
    def _fuse_kernels(self, graph: HyperGraph, pattern: FusionPattern) -> FusionResult:
        ...
        kernels = [v for v in pattern.vertices 
                   if v.vertex_type == VertexType.KERNEL_LAUNCH]
...
def _detect_polyglot_pipeline(graph: HyperGraph) -> list[FusionPattern]:
    ...
    for vertex in graph.vertices:
        ...
        if prov.source_language == "Python":
            ...
            if len(sequence) >= 2:
                languages = [v.metadata.provenance.source_language for v in sequence if v.metadata.provenance]
                ...
                patterns.append(FusionPattern(
                    ...
                    hardware_target=HardwareAffinity.GPU if "CUDA" in languages else HardwareAffinity.CPU,
                ))
...
def _detect_kernel_fusion(graph: HyperGraph) -> list[FusionPattern]:
    ...
    for kernel in graph.topological_order():
        ...
        if not current_group:
            ...
            if (kernel.metadata.hardware_affinity == last.metadata.hardware_affinity and
                _compatible_parallelism(kernel, last)):
...
def _detect_dataflow_fusion(graph: HyperGraph) -> list[FusionPattern]:
    ...
    sql_ops = [v for v in graph.vertices 
...
class CrossLanguageFuser:
    ...
    def fuse_with_zero_copy(
        ...
        transfers = [v for v in optimized.vertices 
                     if "memcpy" in v.attributes.get("function", "").lower() or
                        "copy" in v.attributes.get("function", "").lower()]
