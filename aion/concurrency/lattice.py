...
class FunctionEffect:
    ...
    def serialize(self) -> dict[str, Any]:
        ...
        return {
            ...
            "required_caps": [{"effect": c.effect.name, "region": c.region} for c in self.required_caps],
            "granted_caps": [{"effect": c.effect.name, "region": c.region} for c in self.granted_caps],
            "pure": self.pure,
        }
            ...
            for w in region_writes:
                for r in region_reads:
                    if w.id != r.id:
                        ...
                        has_ordering = any(
                            (w in e.sources and r in e.targets) or
                            (r in e.sources and w in e.targets)
...
def analyze_deadlocks(graph: Any) -> DeadlockAnalysis:
    ...
    sync_vertices = [
        ...
        if any(e in v.metadata.effects for e in (
