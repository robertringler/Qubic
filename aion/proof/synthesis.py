...
from .verifier import ProofTerm, ProofKind, SafetyTheorem
...
class SMTSolver:
    ...
    def assert_eq(self, name: str, a: str, b: str) -> None:
        """Assert a = b."""
        self.constraints.append(SMTConstraint(
            name=name,
            formula=f"(= {a} {b})",
...
class ProofSynthesizer:
    ...
    def _synthesize_memory_safety(self, graph: Any) -> ProofTerm | None:
        ...
        # Check memory safety
        valid = all(
            use["program_point"] > next(
                (a["program_point"] for a in evidence["allocations"] 
                 if a["id"] == use["alloc_id"]),
                0
            )
        ...
        for edge in parallel_edges:
            ...
            for v in parallel_vertices:
                if EffectKind.READ in v.metadata.effects:
                    accesses.append({"vertex": v.id, "read_only": True, "region": v.metadata.region})
                if EffectKind.WRITE in v.metadata.effects:
                    accesses.append({"vertex": v.id, "read_only": False, "region": v.metadata.region})
    ...
    def _synthesize_deadlock_freedom(self, graph: Any) -> ProofTerm | None:
        ...
        sync_vertices = [
            ...
            if any(e.name in ("THREAD_JOIN", "CHANNEL_RECV", "BARRIER", "ATOMIC_RMW")
                   for e in v.metadata.effects if hasattr(e, 'name'))
        ]
    ...
    def _synthesize_bounded_resources(self, graph: Any) -> ProofTerm | None:
        ...
        all_bounded = all(
            a.get("bound") != "unbounded"
            for a in evidence["allocations"]
        )
...
def _rewrite_preserves_property(old_graph: Any, new_graph: Any, kind: ProofKind) -> bool:
    ...
    if kind == ProofKind.MEMORY_SAFETY:
        ...
        old_allocs = {v.id for v in old_graph.vertices 
                      if hasattr(v, 'vertex_type') and v.vertex_type.name == 'ALLOC'}
        new_allocs = {v.id for v in new_graph.vertices 
                      if hasattr(v, 'vertex_type') and v.vertex_type.name == 'ALLOC'}
