...
class LLVMType:
    ...
    def ptr(element: LLVMType) -> LLVMType:
        return LLVMType(name="ptr", is_pointer=True, element_type=element, size=64)
...
class LLVMInstruction:
    ...
    def to_ir(self) -> str:
        ...
        elif self.opcode in ("add", "sub", "mul", "sdiv", "udiv", "srem", "urem") or self.opcode in ("fadd", "fsub", "fmul", "fdiv", "frem"):
...
class LLVMCodeGen:
    ...
    def _add_region_metadata(self, graph: HyperGraph) -> None:
        ...
        for i, region in enumerate(sorted(regions)):
            self.module.metadata.append(f'!{i} = !{{!"region", !"{region}"}}')
