...
def main():
    ...
    while True:
        ...
        if status['dm_done'] and status['traj_done']:
            print(f"[{now}] ALL DONE — both full and trajectory runs finished. Artifacts present. ✔")
            break

        if time.time() - start > timeout_s:
            print(f"[{now}] Watcher timeout reached. Current status: dm_done={status['dm_done']} traj_done={status['traj_done']}")
            break
