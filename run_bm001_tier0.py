...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of runs per solver (default: 5)"
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    try:
        ...
        logger.info(f"  Speedup 95% CI: [{stats['speedup_ci_lower']:.2f}, {stats['speedup_ci_upper']:.2f}]")
        logger.info(f"  Ansys outliers: {stats['ansys_outliers']}")
        logger.info(f"  QuASIM outliers: {stats['quasim_outliers']}")
        logger.info(f"  Statistical significance: {stats['significance']} (p={stats['p_value']:.3f})")
    ...
    if len(unique_hashes) == 1:
        ...
        logger.warning(f"  Found {len(unique_hashes)} unique hashes across {len(quasim_results)} runs")
