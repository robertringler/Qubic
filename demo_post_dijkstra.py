...
def main():
    ...
    parser = argparse.ArgumentParser(description="PostDijkstra SSSP Algorithm Demonstration")
    ...
    parser.add_argument(
        "--delta", type=float, default=None, help="Bucket width (default: auto-compute)"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--no-hierarchy", action="store_true", help="Disable hierarchical decomposition"
    )
    parser.add_argument(
        "--no-lower-bounds", action="store_true", help="Disable lower-bound pruning"
    )
    parser.add_argument(
        "--no-validate", action="store_true", help="Skip validation against Dijkstra"
    )
    ...
    graph = QGraph.random_graph(
        num_nodes=args.nodes, edge_probability=args.edge_prob, seed=args.seed
    )
