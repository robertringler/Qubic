...
class PersistenceDiagram:
    ...
    def get_betti_numbers(self, threshold: float = 0.0) -> BettiNumbers:
        ...
        """
        beta_0 = sum(1 for i in self.dimension_0_intervals if i.birth <= threshold < i.death)
        beta_1 = sum(1 for i in self.dimension_1_intervals if i.birth <= threshold < i.death)
        beta_2 = sum(1 for i in self.dimension_2_intervals if i.birth <= threshold < i.death)
    ...
    def compute_bottleneck_distance(self, other: PersistenceDiagram) -> float:
        ...
        if len(self.intervals) == 0 or len(other.intervals) == 0:
            max_pers = max(
                (
                    i.persistence
                    for i in self.intervals + other.intervals
