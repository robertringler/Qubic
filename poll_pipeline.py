...
def poll(run_dir: Path, interval: float, follow: bool, tail: int, json_out: bool, show_active_tail: bool):
    first = True
    while True:
        ...
        if json_out:
            ...
            print(f"Status: {summary['status']}  Completed: {summary['task_completed']}/{summary['task_total']}  Failures: {summary['task_failures']}")
            ...
            if summary['status'] in ('success', 'failed') and summary['index_present']:
                ...
                if isinstance(idx, dict):
                    gate_keys = [k for k in idx.keys() if k.endswith('_gate_ok') or k.endswith('_fraction')]
                    if gate_keys:
                        print("Gates:")
...
def parse_args():
    ap = argparse.ArgumentParser(description='Poll an automation pipeline run directory for progress.')
    ap.add_argument('--root', type=str, default='automation_runs', help='Root directory containing run hash subdirectories.')
    ap.add_argument('--run-dir', type=str, default=None, help='Specific run directory (overrides automatic latest detection).')
    ap.add_argument('--interval', type=float, default=10.0, help='Polling interval seconds when --follow enabled.')
    ap.add_argument('--follow', action='store_true', help='Continue polling until run completes.')
    ap.add_argument('--tail', type=int, default=12, help='Number of lines to tail from active task stdout.')
    ap.add_argument('--no-active-tail', action='store_true', help='Disable tailing active stdout.')
    ap.add_argument('--json', action='store_true', help='Emit JSON summary (single shot or each interval).')
    ap.add_argument('--once', action='store_true', help='Perform a single poll iteration and exit with status code.')
