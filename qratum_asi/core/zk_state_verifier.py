...
class ZKStateVerifier:
    ...
    def verify_proof(
        self, proof: ZKProof, commitment: StateCommitment, context: ZKVerificationContext
        ...
        if context.trajectory_state in [TrajectoryState.CRITICAL, TrajectoryState.SELF_SUSPEND]:
            ...
            return (
                VerificationResult.TRAJECTORY_VIOLATION,
                f"System in {context.trajectory_state.value} state",
            )
        ...
        if proof_age > context.max_proof_age:
            ...
            return (
                ...
                f"Proof expired (age: {proof_age}s, max: {context.max_proof_age}s)",
            )
        ...
        if self.replay_cache.is_replay(proof_id):
            ...
            return (
                ...
                f"Replay attack detected (proof_id: {proof_id[:16]}...)",
            )
    ...
    def verify_transition(
        self, transition: ZKStateTransition, context: ZKVerificationContext
    ) -> Tuple[VerificationResult, str]:
        ...
        if transition.transition_type not in self.verifying_keys:
            ...
            return (
                VerificationResult.FORMAT_ERROR,
                f"No verifying key for {transition.transition_type.value}",
            )
    ...
    def _verify_proof_cryptographic(self, proof: ZKProof, commitment: StateCommitment) -> bool:
...
class ZKProofGenerator:
    ...
    def generate_proof(
        ...
        proof_data = self._generate_proof_data(prev_state, next_state, transition_witness, nonce)
    ...
    def _generate_proof_data(
        self, prev_state: bytes, next_state: bytes, witness: bytes, nonce: bytes
    ) -> bytes:
    ...
    def create_transition(
        ...
        proof = self.generate_proof(prev_state, next_state, transition_witness, transition_type)
...
def verify_state_transition(
    prev_state: bytes, next_state: bytes, proof: ZKProof, zone_id: str, state_version: int
) -> Tuple[bool, str]:
