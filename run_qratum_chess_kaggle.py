...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--competition", "-c",
        type=str,
        help="Kaggle competition ID"
    )
    
    parser.add_argument(
        "--input", "-i",
        type=str,
        help="Input CSV file with positions (if not downloading)"
    )
    ...
    parser.add_argument(
        "--enable-trimodal",
        action="store_true",
        help="Enable tri-modal cortex fusion"
    )
    
    parser.add_argument(
        "--enable-novelty-pressure",
        action="store_true",
        help="Enable novelty pressure"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Novelty pressure value (0.0-1.0)"
    )
    
    parser.add_argument(
        "--disable-randomness",
        action="store_true",
        help="Disable randomness for reproducibility"
    )
    
    parser.add_argument(
        "--submit",
        action="store_true",
        help="Submit to Kaggle after generation"
    )
    ...
    parser.add_argument(
        "--track",
        action="store_true",
        help="Track leaderboard ranking"
    )
    
    parser.add_argument(
        "--no-mock",
        action="store_true",
        help="Enforce no mock data (real competition only)"
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"✓ Kaggle API client initialized")
    except Exception as e:
    ...
    if args.input:
        ...
        try:
            dataset_info = client.download_competition_data(
                args.competition,
                data_dir
            )
            ...
            # Find test file
            test_files = [f for f in dataset_info.files if 'test' in f.lower() and f.endswith('.csv')]
            if not test_files:
                print(f"✗ Error: No test CSV found in dataset")
                return 1
        ...
        print(f"✗ Error: Must specify either --competition or --input")
        return 1
    
    if not positions:
        print(f"✗ Error: No positions to process")
        return 1
    ...
    # Run engine
    moves, evaluations, engine_stats = run_engine_on_positions(
        engine,
        positions,
        params
    )
    ...
    submission_file = formatter.format_submission(
        predictions,
        output_file,
        metadata=metadata
    )
    ...
    if args.submit:
        if not args.competition:
            print(f"✗ Error: --competition required for submission")
            return 1
